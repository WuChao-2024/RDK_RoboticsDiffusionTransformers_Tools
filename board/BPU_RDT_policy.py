...
AGILEX_STATE_INDICES  = [STATE_VEC_IDX_MAPPING[f"left_arm_joint_{i}_pos"] for i in range(6)]
AGILEX_STATE_INDICES += [STATE_VEC_IDX_MAPPING["left_gripper_open"]]
AGILEX_STATE_INDICES += [STATE_VEC_IDX_MAPPING[f"right_arm_joint_{i}_pos"] for i in range(6)]
AGILEX_STATE_INDICES += [STATE_VEC_IDX_MAPPING[f"right_gripper_open"]]
...
class BPU_RDT_Policy():
    def __init__(self, bpu_rdt_path, config, SERVER_URL=None):
        ...
        self.siglip = CauchyKesai(os.path.join(bpu_rdt_path, "bpu_siglip_so400m_patch14_nashm_384x384_featuremaps.hbm"), n_task=6)
        self.siglip.s()
    ...
    def step(self, imgs, joints):
        # imgs: list[np.array], (480, 640, 3), np.uint8, RGB, 如果是None, 请处理到(480, 640, 3)大小, 颜色(127,127,127)的图
        # joints: np.array, (14,), np.float32
